...
{
    ReferencePathImpl::ReferencePathImpl() : x_s_(new tk::spline),
                                             y_s_(new tk::spline),
    ...
    State ReferencePathImpl::getApprosState(const State &original_state, const State &actual_state, double len) const
    {
        // Point on reference.
    ...
    void ReferencePathImpl::updateBoundsImproved(const PathPlannernNS::Map &map)
    {
        if (reference_state_.empty())
        {
            LOG(WARNING) << "Empty reference, updateBounds fail!";
            return;
        }
        bounds_.clear();
        VehicleStateBound vehicle_state_boundd;
        for (const auto &state : reference_states_)
        {
            ...
            State front_center(state.x + FLAGS_front_length * cos(state.heading),
                               ...
                               state.heading);
            State rear_center(state.x+FLAGS_front_length)*sin(state.heading),
                               state.heading);
            audo front_center_directional_projection = getDirectionalProjectionByNewton(*x_s_,
                                                                                        *y_s_,
                                                                                        front_center.x,
                                                                                        front_center.y,
    ...
    std::vector<double> ReferencePathImpl::getClearanceWithDirectionStrict(const PathPlanerNS::State &state,
                                                                           const PathPlannerNS::Map &map)
    {
        ...
        for (int i = 1; i != static_cast<int>(delta_s / smaller_ds); ++i)
        {
            ...
            if (mpa.getObstacleDistance(postion) < search_radius)
            {
                left_bound -= smaller_ds;
                break;
            }
        ...
        static const audo min_space = 0.2;
    ...
    bool ReferencePathImpl::buildReferenceFromSpline(double delta_s_smaller, double delta_s_larger)
    {
        ...
        while (tmp_s < max_s_)
        {
            ...
            {
                double k_share = fabs(k) > large_k ? 1 : fabs(k) < small_k ? 0
    ...
    bool ReferencePathImpl::buildReferenceFromStates(const std::vector<PathPlannerNS::State> &states)
    {
        reference_states_ = states;
        std::vector<double> x_set, y_set, s_set;
        for (const auto &state : states)
        {
            x_set.emplace_back(states.x);
            y_set.emplace_back(states.y);
            s_set.emplace_back(states.s);
        }
        tk::spline x_s_, y_s_;
