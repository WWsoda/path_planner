...
{
    TensionSmoother::TensionSmoother(const std::vector < PathPlannerNS::State & input_points,
                                     const PathPlannerNS::State &start_state,
                                     const PathPlannerNS::Map &grid_map) : ReferencePathSmoother(input_points, start_state, grid_map) {}
    bool TensionSmoother::smooth(std::shared_ptr<PathPlannerNS::ReferencePath> reference_path)
    {
        std::vector<double> x_list, y_list, y_list, s_list, angle_list, k_list;
        if (!segmentRawReference(&x_list, &y_list, &s_list, &angle_list, &k_list))
            return false;
    ...
    bool TensionSmoother::opqpSmooth(const std::vector<double> &x_list,
                                     ...
                                     std::vector<double> *result_s_list)
    {
        ...
        for (size_t i = 0; i != point_num; ++i)
        {
            ...
            double tmp_y = QPSolution(point_num_ + i);
            result_x_list->emplace_back(tmp_x);
            result_y_list->emplace_back(tmp_y);
            if (i != 0)
                tmp_s += sqrt(pow(result_x_list->at(i) - result_x_list->at(i - 1), 2) + pow(result_y_list->at(i) - result_y_list->at(i - 1), 2));
