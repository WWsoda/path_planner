...
#ifndef PATH_PLANNER_INCLUDE_REFERENCE_PATH_PLANNER_REFERENCE_PATH_SMOOTHER_HPP_
#define PATH_PLANNER_INCLUDE_REFERENCE_PATH_PLANNER_REFERENCE_PATH_SMOOTHER_HPP_
#include <iostream>
#include <memory>
#include <vector>
...
namespace PathPlannerNS
{
    class Map;
    class ReferencePath;
    // This class uses searching method to improve the quality of
    // the input points (if needed), and
    ...
    public:
        ...
        bool solve(std::shared_ptr<PathPlannerNS::ReferencePath> reference_path);
    ...
    private:
        virtual bool smooth(std::shared_ptr<PathPlannerNS::ReferencePath> reference_path) = 0;
        void bSpline();
        bool postSmooth(std::shared_ptr<PathPlannerNS::ReferencePath> reference_path);
        void setPostHessianMatrix(Eigen::SparseMatrix<double> *matrix_constraints,
                                  Eigen::VectorXd *lower_bound,
                                  Eigen::VectorXd *upper_bound) const;
        // search
        bool graphSEarch(std::shared_ptr<PathPlannerNS::ReferencePath> reference);
        boolgraphSearchDp(std::shared_ptr<PathPlannerNS::ReferencePath> reference);
        void calculateCostAt(std::vector<std::vector<DpPoint>> &samples, int layer_index, int lateral_index) const;
        inline bool sheckExistenceInClosedSet(const APoint &parent) const;
...
#endif // PATH_PLANNER_INCLUDE_REFERENCE_PATH_SMOOTHER_REFERENCE_PATH_SMOOTHER_HPP_